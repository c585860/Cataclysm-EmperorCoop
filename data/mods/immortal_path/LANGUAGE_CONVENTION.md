...
7. **英文译名以 [GLOSSARY](GLOSSARY.md) 为准。** 术语表是全 mod 中英对照的唯一权威来源；新术语先在术语表登记，再写进 JSON，避免同一概念出现多个译名。
   English term names follow [GLOSSARY](GLOSSARY.md) — the single source of truth for translations. Register a new term there before using it in JSON.
